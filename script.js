...
// Centralized fixed values for the hidden audio/transparency controls.
const PROFILE_SETTINGS = Object.freeze({
  audioVolume: 0.3,
  panelOpacity: 0.4,
  panelBlurMultiplier: 15
});

function isAudioMuted() {
  return PROFILE_SETTINGS.audioVolume <= 0;
}

function applyFixedAudioSettings(audioElements) {
  ...
  audioElements.forEach((audio) => {
    ...
    audio.volume = PROFILE_SETTINGS.audioVolume;
    audio.muted = muted;
  });
}

function applyFixedPanelStyles({
  profileBlock,
  skillsBlock,
  socialIcons,
  badges,
  ...
  const opacity = PROFILE_SETTINGS.panelOpacity;
  const backgroundColor = `rgba(0, 0, 0, ${opacity})`;
  const blurValue = opacity === 0 ? 'none' : `blur(${PROFILE_SETTINGS.panelBlurMultiplier * opacity}px)`;
  const borderColor = opacity === 0 ? 'transparent' : '';
  ...
  [profileBlock, skillsBlock].forEach((block) => {
    ...
    block.style.backdropFilter = blurValue;
  ...
  [profileName, profileBio, visitorCount].forEach((element) => {
    if (element) {
...
function initMedia() {
  ...
  applyFixedAudioSettings([backgroundMusic, hackerMusic, rainMusic, animeMusic, carMusic]);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bioMessages = [
    "AcronDLC - The Best DLC in the world",
    "\"Hello, World!\""
  ];
  let bioText = '';
  let bioIndex = 0;
  ...
  function switchTheme(videoSrc, audio, themeClass, overlay = null, overlayOverProfile = false) {
    ...
    gsap.to(backgroundVideo, {
      ...
      onComplete: () => {
        ...
        currentAudio = audio;
        currentAudio.volume = PROFILE_SETTINGS.audioVolume;
